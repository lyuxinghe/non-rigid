...
from non_rigid.models.dit.models import (
    TAX3Dv2_No_Track_DiT,
)
from non_rigid.utils.logging_utils import viz_predicted_vs_gt
from non_rigid.utils.pointcloud_utils import expand_pcd


def TAX3Dv2_No_Track_DiT_xS(**kwargs):
    return TAX3Dv2_No_Track_DiT(depth=5, hidden_size=128, num_heads=4, **kwargs)

DiT_models = {
    "TAX3Dv2_No_Track_xS": TAX3Dv2_No_Track_DiT_xS,
}

def get_model(model_cfg):
    return DiT_models["TAX3Dv2_No_Track_xS"]

class TAX3Dv2NoTrackNetwork(L.LightningModule):
    """
...
class TAX3Dv2NoTrackModule(L.LightningModule):
    ...
    def predict_wta(self, batch, num_samples):
        ...
        Args:
            ...
            num_samples: the number of samples to generate
        """
        raise NotImplementedError("Tracking-free model cannot compute WTA metrics.")
    ...
    def log_viz_to_wandb(self, batch, pred_dict, tag):
        ...
        """
        bs, num_points, _ = batch["pc"].shape
        pred_point_world = pred_dict["point"]["pred_world"].reshape(bs, -1, num_points, 3)
        # pick a random sample in the batch to visualize
        viz_idx = np.random.randint(0, bs)
        ...
        pred_action_viz = pred_point_world[viz_idx, 0, :, :3]
        viz_args["pred_action_viz"] = pred_action_viz
        predicted_vs_gt = viz_predicted_vs_gt(**viz_args)
        wandb.log({f"{tag}/predicted_vs_gt_1": predicted_vs_gt})

        # # logging predicted vs ground truth point cloud, again
        pred_action_viz = pred_point_world[viz_idx, 1, :, :3]
        viz_args["pred_action_viz"] = pred_action_viz
        predicted_vs_gt = viz_predicted_vs_gt(**viz_args)
        wandb.log({f"{tag}/predicted_vs_gt_2": predicted_vs_gt})
    ...
    def training_step(self, batch):
        ...
        if do_additional_logging:
            ...
            with torch.no_grad():
                ...
                pred_dict = self.predict(batch, 2, unflatten=False, progress=True)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        ####################################################
        # logging dummy validation wta metrics
        ####################################################
        self.log_dict(
            {
                f"val_rmse_{dataloader_idx}": 0,
                f"val_rmse_wta_{dataloader_idx}": 0,
            },
            add_dataloader_idx=False,
            prog_bar=True,
        )
