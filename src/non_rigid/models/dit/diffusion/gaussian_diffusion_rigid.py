...
class GaussianDiffusionRigid:
    ...
    def training_losses(self, model, xr_start, xs_start, t, model_kwargs=None, noise=None):
        """
        Compute training losses for a single timestep using separate forward processes for
        the reference (R) and shape (S) components.
        
        xr_start: [B, 3, 1] ground-truth t.
        xs_start: [B, 5, 1] ground-truth R in 6d orthogonal representation.
        ...
        noise_r = th.randn_like(xr_start)  # noise for R: [B, 3, 1]
        noise_s = th.randn_like(xs_start)  # noise for S: [B, 6, 1]
        '''
        ...
        xr_t = self.q_sample(xr_start, t, noise=noise_r)  # [B, 3, 1]
        xs_t = self.q_sample(xs_start, t, noise=noise_s)  # [B, 6, 1]
    ...
    def calc_bpd_loop(self, model, x_start, clip_denoised=True, model_kwargs=None):
        ...
        for t in list(range(self.num_timesteps))[::-1]:
            ...
            raise NotImplementedError("out[\"pred_xstart\"] not implemented!")
