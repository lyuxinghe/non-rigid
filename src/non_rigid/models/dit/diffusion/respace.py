...
from .gaussian_diffusion_rigid import GaussianDiffusionRigid
...
class SpacedDiffusionRigid(GaussianDiffusionRigid):
    """
    A diffusion process which can skip steps in a base diffusion process.
    :param use_timesteps: a collection (sequence or set) of timesteps from the
                          original diffusion process to retain.
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        base_diffusion = GaussianDiffusionRigid(**kwargs)  # pylint: disable=missing-kwoa
        last_alpha_cumprod = 1.0
        new_betas = []
        for i, alpha_cumprod in enumerate(base_diffusion.alphas_cumprod):
            if i in self.use_timesteps:
...
class _WrappedModelRS:
    ...
    def __call__(self, xr, xs, ts, **kwargs):
        map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype)
        new_ts = map_tensor[ts]
        # if self.rescale_timesteps:
        #     new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
        return self.model(xr, xs, new_ts, **kwargs)